import {useState} from 'react'

const Button = (props) => {
  return (
    <button onClick={props.handleClick}>
    {props.text}
    </button>
  )
}


const Header = (props) => {
  return (
    <h2>
      {props.text}
    </h2>
  )
}

const MostVotesAnecdote = (props) => {
  let index = props.votes.indexOf(Math.max(...props.votes))
  return (
    <div>
    <p>
      {props.anecdotes[index]}
    </p>
    <p>
      Has {props.votes[index]} votes
    </p>
    </div>
  )
}


const anecdotes = [
  'If it hurts, do it more often.',
  'Adding manpower to a late software project makes it later!',
  'The first 90 percent of the code accounts for the first 10 percent of the development time...The remaining 10 percent of the code accounts for the other 90 percent of the development time.',
  'Any fool can write code that a computer can understand. Good programmers write code that humans can understand.',
  'Premature optimization is the root of all evil.',
  'Debugging is twice as hard as writing the code in the first place. Therefore, if you write the code as cleverly as possible, you are, by definition, not smart enough to debug it.',
  'Programming without an extremely heavy use of console.log is same as if a doctor would refuse to use x-rays or blood tests when diagnosing patients.',
  'The only way to go fast, is to go well.'
]

const points = Array(anecdotes.length).fill(0)

const App = () => {

  const [selected, setSelected] = useState(0)
  const [votes, setVotes] = useState(points)

  const clickHandler = () => {
    let x = Math.floor(Math.random() * anecdotes.length)
    console.log("Value of x within clickHandler ", x)
    setSelected(x)
  }

  const clickHandlerVote = (indexAnecdote) => {
    console.debug("Value of indexAnecdote", indexAnecdote)
    const copy = [...votes]
    copy[indexAnecdote] += 1
    console.debug("Value of array entry", copy[indexAnecdote])
    setVotes(copy)
  }

  return (
    <div className="App">
      <Header text="Anecdote of the day" />
      <div>
      {anecdotes[selected]}
      </div>
      <div>
      Has {votes[selected]} votes
      </div>
      <Button handleClick={() => clickHandlerVote(selected)} text="vote"/>
      <Button handleClick={() => clickHandler()} text="next anecdote" />
      <Header text="Anecdote with most votes"/>
      <MostVotesAnecdote votes={votes} anecdotes={anecdotes}/>
    </div>
  );
}

export default App;
